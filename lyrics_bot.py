...
def is_arabic_query(s: str) -> bool:
    # لو في أحرف عربية كفاية أو نسبة العربي عالية
    arab = len(AR_RANGE.findall(s))
    return arab >= 2 or (arab / max(len(s), 1) >= 0.2)

def split_artist_title(q: str) -> Tuple[str, str]:
    q_clean = normalize_ar(q)
    m = re.split(r"\s[-–—]\s|[-–—]|:|\|", q)
    if len(m) >= 2:
        left = normalize_ar(m[0]); right = normalize_ar(" ".join(m[1:]))
        return left, right
    parts = q_clean.split()
    if len(parts) >= 3:
        title = " ".join(parts[-2:]); artist = " ".join(parts[:-2])
        return artist, title
    return "", q_clean
...
async def provider_lyricstranslate_ar(session, query: str) -> Optional[Tuple[str, str]]:
    """
    LyricsTranslate (Arabic results): نبحث ثم نجلب نص من كتلة lyrics.
    """
    # بحث داخلي بسيط
    ...
    # الترتيب: أول نتيجة عربية
    for a in soup.select("a[href*='/ar/song/'], a[href*='/ar/%D8%A3%D8%BA%D9%86%D9%8A%D8%A9/']"):
        song_url = "https://lyricstranslate.com" + a.get("href", "")
        page = await fetch_text(session, song_url)
        if not page:
            continue
        sp = BeautifulSoup(page, "html.parser")
        block = sp.select_one(".lyrics, .lt-lyrics, #song-body, .song-node .field-lyrics") or sp.find("div", class_="lyrics")
        if not block:
            continue
...
async def provider_arabiclyrics(session, query: str) -> Optional[Tuple[str, str]]:
    """
    ArabicLyrics.net: بحث بسيط + التقاط كتلة المحتوى.
    """
    q = "+".join(query.split())
